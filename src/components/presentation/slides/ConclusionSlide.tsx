...
const ConclusionSlide = () => (
  <SlideLayout>
    <div className="flex flex-col items-center justify-center h-full text-center">
      ...
      <p className="text-2xl max-w-[1200px] mb-16 leading-relaxed" style={{ color: "hsl(215 15% 60%)" }}>
        Secure-Drop proves that enterprise-grade, secure file transfer doesn't require 
        complex software installations or third-party cloud subscriptions.
      </p>
      ...
      <div className="grid grid-cols-2 gap-8 max-w-[1400px] w-full mb-16">
        {points.map((p, i) => (
          <div
            key={i}
            className="flex items-center gap-6 p-8 rounded-xl"
            style={{
              background: "hsl(220 20% 12% / 0.8)",
              border: "1px solid hsl(170 85% 50% / 0.2)",
            }}
          >
            <p.icon size={36} style={{ color: "hsl(170 85% 50%)" }} />
            <span className="text-xl text-left">{p.text}</span>
          </div>
        ))}
      </div>
      ...
      <div
        ...
        style={{
          background: "linear-gradient(135deg, hsl(170 85% 50% / 0.15), hsl(145 70% 50% / 0.15))",
          border: "1px solid hsl(170 85% 50% / 0.3)",
        }}
      >
        <p className="text-3xl font-bold" style={{ color: "hsl(170 85% 50%)" }}>
